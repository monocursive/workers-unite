defmodule WorkersUnite.Agent do
  @moduledoc """
  GenServer representing an AI agent in the WorkersUnite network.
  ...
  alias WorkersUnite.Agent.TaskContext
  ...
  def spawn(kind, opts \\ []) do
    ...
    case Horde.DynamicSupervisor.start_child(WorkersUnite.AgentSupervisor, child_spec) do
  ...
  defp via(agent_id) do
    {:via, Horde.Registry, {WorkersUnite.Registry, {__MODULE__, agent_id}}}
  end
      ...
      true ->
        with {:ok, task_context} <- TaskContext.resolve(state),
             {:ok, session_pid, session_token} <-
               WorkersUnite.Agent.Session.start_for_agent(
                 self(),
  ...
  def handle_call({:publish_intent, repo_id, payload}, _from, state) do
    case WorkersUnite.Repository.publish_intent(repo_id, state.keypair, payload) do
  ...
  defp maybe_reserve_claim(repo_id, intent_ref, agent_id) do
    WorkersUnite.Repository.reserve_intent_claim(repo_id, intent_ref, agent_id)
  end

  defp maybe_release_claim(nil, _intent_ref, _agent_id), do: :ok

  defp maybe_release_claim(repo_id, intent_ref, agent_id) do
    WorkersUnite.Repository.release_intent_claim(repo_id, intent_ref, agent_id)
  end
