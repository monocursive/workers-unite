defmodule WorkersUnite.Credentials.Encryption do
  @moduledoc """
  ...
  defp encryption_key do
    case Application.get_env(:workers_unite, :credential_encryption_key) do
      key when is_binary(key) and byte_size(key) == 32 ->
        key

      _ ->
        secret_key_base =
          Application.get_env(:workers_unite, WorkersUniteWeb.Endpoint)[:secret_key_base] ||
            raise "No credential_encryption_key or secret_key_base configured"

        :crypto.mac(:hmac, :sha256, secret_key_base, "workers_unite_credential_encryption")
    end
  end
end
