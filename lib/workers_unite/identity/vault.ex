defmodule WorkersUnite.Identity.Vault do
  @moduledoc """
  ...
  def start_link(opts) do
    ...
    key_path =
      Keyword.get(opts, :key_path, Application.app_dir(:workers_unite, "priv/identity/node.key"))
  ...
  def init(%{key_path: key_path}) do
    keypair =
      if File.exists?(key_path) do
        ...
        kp = WorkersUnite.Identity.generate()
  ...
  def handle_call(:fingerprint, _from, %{keypair: %{public: pub}} = state) do
    {:reply, WorkersUnite.Identity.fingerprint(pub), state}
  end
