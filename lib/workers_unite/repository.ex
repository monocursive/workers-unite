defmodule WorkersUnite.Repository do
  @moduledoc """
  GenServer representing a code repository in the WorkersUnite network.

  Each repository is a Horde-distributed process registered via
  `WorkersUnite.Registry` and supervised by `WorkersUnite.RepoSupervisor`.
  ...
  alias WorkersUnite.{Event, EventStore, Git}
  ...
  @doc """
  Creates a new repository, starts it under `WorkersUnite.RepoSupervisor`, and
  ...
  def create(name, owner_keypair, opts \\ []) do
    ...
    case Horde.DynamicSupervisor.start_child(WorkersUnite.RepoSupervisor, child_spec) do
  ...
  @doc """
  Lists all repositories currently registered in `WorkersUnite.Registry`.
  """
  def list_local do
    Horde.Registry.select(WorkersUnite.Registry, [
  ...
  defp via(repo_id) do
    {:via, Horde.Registry, {WorkersUnite.Registry, {__MODULE__, repo_id}}}
  end
  ...
  def init(opts) do
    ...
    repo_base_path = Application.get_env(:workers_unite, :repo_base_path, "priv/repos")
    ...
    Phoenix.PubSub.subscribe(WorkersUnite.PubSub, "events:kind:consensus_reached")
    ...
    policy = Application.get_env(:workers_unite, :default_consensus_policy, {:threshold, 2, 0.7})
  ...
  defp merge_branch_into_main(bare_repo_path, branch_name) do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "workers-unite-merge-#{System.unique_integer([:positive, :monotonic])}"
      )
    ...
    try do
      with :ok <- Git.clone(bare_repo_path, tmp_dir),
           :ok <- Git.configure_identity(tmp_dir, "WorkersUnite", "workers_unite@example.local"),
  ...
  defp ensure_initial_commit(bare_repo_path) do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "workers-unite-bootstrap-#{System.unique_integer([:positive, :monotonic])}"
      )
    ...
    try do
      with {_, 0} <-
             System.cmd("git", ["clone", bare_repo_path, tmp_dir], stderr_to_stdout: true),
           {_, 0} <-
             System.cmd("git", ["config", "user.name", "WorkersUnite"],
               cd: tmp_dir,
               stderr_to_stdout: true
             ),
           {_, 0} <-
             System.cmd("git", ["config", "user.email", "workers_unite@example.local"],
               cd: tmp_dir,
               stderr_to_stdout: true
             ),
           {_, 0} <-
