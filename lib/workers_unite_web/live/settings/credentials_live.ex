defmodule WorkersUniteWeb.Settings.CredentialsLive do
  use WorkersUniteWeb, :live_view

  @moduledoc """
  Manages encrypted API credentials for AI model providers. Supports creating,
  ...
  def handle_event("save_credential", %{"credential" => params}, socket) do
    ...
    case Credentials.upsert(params["provider"], params["key_name"], params["value"], user_id) do
      {:ok, _} ->
        try do
          WorkersUnite.CredentialStore.reload()
        catch
          _, _ -> :ok
        end
  ...
  defp load_providers(socket) do
    ...
    provider_registry = Application.get_env(:workers_unite, :provider_registry, %{})
    model_catalog = Application.get_env(:workers_unite, :opencode_model_catalog, [])

    provider_models =
      Enum.group_by(model_catalog, fn entry -> entry.provider end)

    providers =
      Enum.map(provider_registry, fn {name, config} ->
        ...
        models = Map.get(provider_models, name, [])

        %{name: name, keys: cred_keys, stored: stored, status: status, models: models}
      end)
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="space-y-6">
        ...
        <p class="text-sm opacity-70">
          Configure API keys for model providers. Each provider's key is required to use models from that provider.
        </p>

        <div :for={provider <- @providers} class="card bg-base-200 p-4 space-y-3">
          <div class="flex items-center gap-3">
            <h2 class="font-semibold">{format_provider_name(provider.name)}</h2>
            <span class={"badge badge-sm #{status_badge(provider.status)}"}>
              {provider.status}
            </span>
          </div>

          <div :if={provider.models != []} class="text-xs opacity-60">
            Models: {Enum.map_join(provider.models, ", ", & &1.label)}
          </div>
  ...
  defp format_provider_name(:azure), do: "Azure"
  defp format_provider_name(name) when is_atom(name), do: format_provider_name(to_string(name))

  defp format_provider_name(name) do
    name
    |> String.replace("_", " ")
    |> String.split(" ")
