defmodule WorkersUniteWeb.ConsensusLive do
  ...
  alias WorkersUnite.{EventStore, Identity}
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      ...
      Phoenix.PubSub.subscribe(WorkersUnite.PubSub, "events:kind:proposal_submitted")
    end
    ...
    active_proposals =
      proposals
      |> Enum.reject(fn p -> MapSet.member?(decided_refs, WorkersUnite.Event.ref(p)) end)
  ...
  def handle_info({:event, %{kind: kind} = event}, socket)
    ...
    active_proposals =
      if kind == :proposal_submitted do
        if MapSet.member?(decided_refs, WorkersUnite.Event.ref(event)),
        ...
        Enum.reject(socket.assigns.active_proposals, fn p ->
          MapSet.member?(decided_refs, WorkersUnite.Event.ref(p))
        end)
      end
        ...
        <div>
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div class="text-sm">
                <% votes_for = votes_for_proposal(@votes, WorkersUnite.Event.ref(proposal)) %>
                <% accept_count =
