defmodule WorkersUniteWeb.UserAuth do
  @moduledoc """
  ...
  @remember_me_cookie "_workers_unite_web_user_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      WorkersUniteWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  Redirects to onboarding on fresh instances or if onboarding is incomplete.
  """
  def require_authenticated_user(conn, _opts) do
    cond do
      # Fresh instance with no users — go straight to onboarding
      Accounts.first_user?() ->
        conn |> redirect(to: ~p"/onboarding") |> halt()

      # Authenticated but onboarding incomplete
      conn.assigns.current_scope && conn.assigns.current_scope.user &&
          not WorkersUnite.Settings.onboarding_completed?() ->
        conn |> redirect(to: ~p"/onboarding") |> halt()
  ...
  def on_mount(:ensure_authenticated, _params, session, socket) do
    ...
    cond do
      # Fresh instance — redirect to onboarding
      Accounts.first_user?() ->
  ...
  def on_mount(:ensure_authenticated_for_onboarding, _params, session, socket) do
    ...
    cond do
      WorkersUnite.Settings.onboarding_completed?() ->
